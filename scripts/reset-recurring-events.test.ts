...
check("previousOccurrence quotidienne est la veille", () => {
  assert.equal(previousOccurrence("2026-09-05", "Quotidien", []), "2026-09-04");
});

check("previousOccurrence hebdo remonte a la semaine precedente", () => {
  // 07/09/2026 est un lundi.
  assert.equal(previousOccurrence("2026-09-07", "Hebdo", ["Lun"]), "2026-08-31");
});
...
check("previousOccurrence hebdo multi-jours prend la plus proche", () => {
  ...
  assert.equal(previousOccurrence("2026-09-10", "Hebdo", ["Lun", "Jeu"]), "2026-09-07");
});

check("previousOccurrence sans Weekday ne trouve rien", () => {
  assert.equal(previousOccurrence("2026-09-05", "Hebdo", []), null);
});
...
check("Recurrence inconnue est signalee", () => {
  expect(decide(state({ recurrence: "Mensuel" }), "2026-09-05"), "invalid", null);
});
